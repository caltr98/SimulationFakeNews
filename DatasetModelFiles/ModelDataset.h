    ...
    struct UserTrialStats
{
    ...
    void update(const DatasetPost& post) {
        ...
        void print() const {
            ...
            for (size_t i = 0; i < stateCountsPerEpoch.size(); ++i) {
                ...
                for (const auto& pair : stateCountsPerEpoch[i]) {
                    std::cout << " " << stateToString(pair.first) << "=" << pair.second;
                }
                std::cout << std::endl;
            }
            std::cout << std::endl;
    ...
    // Helper functions to convert JSON to struct
    static void from_json(const nlohmann::json& j, Skill& s) {
        j.at(0).get_to(s.skillname);
        j.at(1).get_to(s.skillURI);
    }

    static void from_json(const nlohmann::json& j, Article& a) {
        j.at("title").get_to(a.title);
        j.at("content").get_to(a.content);
        j.at("news-status").get_to(a.news_status);
        j.at("url").get_to(a.url);
        j.at("matches_skills").get_to(a.matches_skills);
        j.at("article_id").get_to(a.article_id);

        std::vector<Skill> arrayOfSkills;
        // Manually parse the articles array
        const auto& skillsarray = j.at("skills");
        for (const auto& skillOfArray : skillsarray) {
            Skill x;
            from_json(skillOfArray,x);
            arrayOfSkills.push_back(x);
        }
        a.articles_skills = arrayOfSkills;
    }
    ...
    static void from_json(const nlohmann::json& j, Person& p) {
        ...
        j.at("bio").get_to(p.bio);



        // Manually parse the articles array
        const auto& articlesArray = j.at("articles");
        p.articles.clear();
        for (const auto& articleJson : articlesArray) {
            Article article;
            from_json(articleJson,article);
            p.articles.push_back(article);
        }
    }

    PostStats collectPostStats() const;
    void collectPostAnalysisAndStoreInCSV(std::string csvName) const;
    static void toCSVPostAnalysis(std::vector<std::string>, std::string csvName);
    UserTrialStats collectUserTrialStats();
    EpochStateStats collectMostPopularStatePerEpoch();

    void analysisPerPostToCSV();

    static std::vector<Person> readPersonsFromFile(const std::string& filename);
    void printEpochStateStats(const EpochStateStats& epoch_state_stats);
    void printPostTrialStats(const PostStats& post_stats);
    ...
    ModelDataset(int num_users, int num_checkers, double prob_susceptible, double prob_recover, double prob_spreader,
                 int epochs, std::string engines, int min_matches);
    ModelDataset(int num_users, int num_checkers, double prob_susceptible, double prob_recover, double prob_spreader,
    ...
    static void printAggregatedEpochStateStats(const EpochStateStatsAggregator stats);
